...
object LobbyOptActor {
  def props = Props(new LobbyOptActor)
}
class LobbyOptActor extends Actor with ActorLogging {
  ...
  def tryMakeMatch = waitingPlayers.length match {
    case x if x == 2 || x == 3 =>
      ...
      val gameRoomRef = context.system.actorOf(GameSupervisorOptActor.props(gameRoom), "game-supervisor")
      ...
      gameRoomRef ! GameSupervisorOptActor.Messages.ReceivePlayers(playersMatched)
